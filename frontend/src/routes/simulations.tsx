  ...
  head: () => ({
    meta: [
      { title: "APEXiq · Race Simulations" },
      {
        name: "description",
        content:
          "Run race strategy, undercut and weather impact simulations through the APEXiq engine.",
      },
    ],
  }),
});
...
function buildSimCharts(result: SimulationApiResponse) {
  const base = 78.4;
  const laps = 30;
...
function SimCard({
  ...
  onRun: () => void;
  loading: boolean;
  result: SimulationApiResponse | null;
}) {
  ...
  const confidence = result
    ? Math.min(
        ...
        Math.max(
          0.2,
          1 -
            Math.abs(result.stay_out_loss - result.pit_loss) /
              Math.max(result.stay_out_loss, result.pit_loss, 1),
        ),
      )
    : 0;
    ...
    >
      <div className="p-5 flex-1 flex flex-col">
        <div className="flex items-center justify-between mb-3">
          <span className="text-[11px] font-semibold text-[#9CA3AF] uppercase tracking-[0.5px]">
            {config.tag}
          </span>
          <span
            className={`w-1.5 h-1.5 rounded-full ${result ? "bg-[#10B981]" : "bg-[#D1D5DB]"}`}
          />
        </div>
        <h3 className="font-inter font-bold text-lg text-[#1A1D29] mb-2">
          {config.title}
        </h3>
        <p className="text-[13px] text-[#6B7280] leading-[1.6] mb-4 flex-1">
          {config.desc}
        </p>
        ...
        <AnimatePresence mode="wait">
          {result ? (
            ...
            >
              <div className="grid grid-cols-2 gap-2">
                <div className="border border-[#E5E7EB] rounded-md p-2.5 bg-white">
                  <div className="text-[11px] font-semibold text-[#6B7280] uppercase tracking-[0.5px] mb-1">
                    Stay Out Loss
                  </div>
                  <div className="font-mono text-lg font-semibold text-[#F59E0B] tabular-nums">
                    {result.stay_out_loss.toFixed(2)}s
                  </div>
                </div>
                <div className="border border-[#E5E7EB] rounded-md p-2.5 bg-white">
                  <div className="text-[11px] font-semibold text-[#6B7280] uppercase tracking-[0.5px] mb-1">
                  ...
                  <div className="font-mono text-lg font-semibold text-[#0EA5E9] tabular-nums">
                    {result.pit_loss.toFixed(2)}s
                  </div>
                </div>
              </div>

              <div
                className={`rounded-md border p-2.5 ${result.undercut_possible ? "border-[rgba(16,185,129,0.2)] bg-[rgba(16,185,129,0.05)]" : "border-[#E5E7EB] bg-white"}`}
              >
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`font-inter text-[13px] font-semibold ${result.undercut_possible ? "text-[#10B981]" : "text-[#9CA3AF]"}`}
                  >
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {[
                  {
                    label: "Stay Out",
                    loss: result.stay_out_loss,
                    color: "text-[#F59E0B]",
                  },
                  {
                    label: "Pit Now",
                    loss: result.pit_loss,
                    color: "text-[#0EA5E9]",
                  },
                ].map((opt) => (
                  <div
                    key={opt.label}
                    className="border border-[#E5E7EB] rounded-md p-2 bg-white"
                  >
                    <div className="text-[11px] font-semibold text-[#6B7280] uppercase tracking-[0.5px] mb-0.5">
                      {opt.label}
                    </div>
                    <div
                      className={`font-mono text-[12px] font-semibold tabular-nums ${opt.color}`}
                    >
            ...
            >
              ...
              <div className="font-mono text-2xl font-semibold text-[#D1D5DB]">
                --
              </div>
            </motion.div>
          )}
...
function SimHistory({ history }: { history: SimHistoryEntry[] }) {
  ...
  return (
    <motion.div
      ...
      className="mt-6"
    >
      <div className="text-[12px] font-semibold text-[#9CA3AF] uppercase tracking-[0.5px] mb-3">
        Simulation History
      </div>
      <div className="space-y-1.5">
        {history
          ...
          .map((entry) => (
            <div
              key={entry.id}
              className="flex items-center gap-3 border border-[#E5E7EB] rounded-md p-2.5 bg-white"
            >
              <div className="w-1.5 h-1.5 rounded-full bg-[#10B981] shrink-0" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-inter text-[13px] font-semibold text-[#1A1D29]">
                    {entry.circuit}
                  </span>
                ...
                <div className="flex items-center gap-3 mt-0.5">
                  <span className="font-mono text-[10px] text-[#F59E0B] tabular-nums">
                    Stay: {entry.result.stay_out_loss.toFixed(2)}s
                  </span>
                  <span className="font-mono text-[10px] text-[#0EA5E9] tabular-nums">
                    Pit: {entry.result.pit_loss.toFixed(2)}s
                  </span>
                  <span
                    className={`font-mono text-[10px] tabular-nums ${entry.result.undercut_possible ? "text-[#10B981]" : "text-[#9CA3AF]"}`}
                  >
...
function SimulationsPage() {
  const [results, setResults] = useState<(SimulationApiResponse | null)[]>([
    null,
    null,
    null,
  ]);
